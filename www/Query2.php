...
<!-- Page Wrapper -->
    ...
    try {
        ...
        //查詢老師有甚麼專長
        //SELECT T.TName, S.Spec 
        //FROM `TEACHER` as T, `Specialization` AS S 
        //WHERE T.TID = S.T_ID AND T.TName = "Lin Yawen" 
        // 檢查是否有使用者輸入的關鍵字
        if (isset($_POST['keyword'])) {
            $keyword = $_POST['keyword'];
            $sql = "SELECT T.TName, S.Spec 
            FROM `TEACHER` as T, `Specialization` AS S 
            WHERE T.TID = S.T_ID AND T.TName = ?";
            ...
            if ($stmt->execute()) {
                ...
                if ($total_records > 0) {
                    ...
                    echo "<br/>";
                    echo "<br/>";
                    echo "<tr><th>Teacher</th><th>Specialization</th></tr>";
    ...
    <form method="post" action="">
        <br/>輸入老師的名稱: <input type="text" name="keyword"  />
        <br/>
